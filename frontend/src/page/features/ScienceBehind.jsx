...
import { Search, Beaker, AlertTriangle, CheckCircle, Info, Lightbulb, Filter, Sparkles, Atom } from 'lucide-react';
...
const ScienceBehind = () => {
  ...
  const foodCombinations = [
    ...
    },
    {
      ...
      benefits: 'Enhanced protein absorption, muscle recovery, sustained energy',
      icon: '🍌🥛',
    ...
    },
    {
      ...
      benefits: 'Enhanced anti-inflammatory effects, better nutrient absorption',
      icon: '🧄🌶️',
      borderColor: 'border-green-400',
      bgColor: 'bg-green-100',
      textColor: 'text-green-700',
    ...
    },
    {
      ...
      sideEffects: 'Increased risk of accidents, heart palpitations, dehydration',
      icon: '🍷☕',
      borderColor: 'border-red-400',
      bgColor: 'bg-red-100',
      textColor: 'text-red-700',
  ...
  const filteredCombinations = useMemo(() => {
    const filtered = foodCombinations.filter(combo => {
      ...
      const matchesFilter = selectedFilter === 'All' || combo.category === selectedFilter;
  ...
  const [currentTip] = useState(didYouKnowTips[Math.floor(Math.random() * didYouKnowTips.length)]);

  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-green-50 relative overflow-hidden">
      ...
      <div className="relative z-10 container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-16 relative">
          ...
          <div className="relative">
            <div className="inline-flex items-center gap-4 mb-6 group">
              ...
              <div>
                ...
                <div className="flex items-center justify-center gap-2 mt-2">
                  ...
                  <Atom className="w-4 h-4 text-blue-500 animate-spin" style={{ animationDuration: '4s' }} />
            ...
            <p className="text-slate-500 text-xl md:text-2xl max-w-3xl mx-auto leading-relaxed font-light">
              Understand the science behind the meals you eat. Learn what combinations work, what don't—and why!
            </p>
          </div>
        </div>
              ...
              <button
                ...
                className={`px-6 py-3 rounded-full text-sm font-semibold transition-all duration-300 flex items-center gap-2 transform hover:scale-105 ${
                  selectedFilter === filter
                    ? 'bg-gradient-to-br from-[#b7f382] via-[#a2db46] to-[#3dd39c] text-white shadow-lg shadow-green-300/30 scale-105'
              ...
              >
                ...
                {filter !== 'All' && (
                  <span className="ml-1 text-xs bg-white/20 px-2 py-0.5 rounded-full">
                    {foodCombinations.filter(c => c.category === filter).length}
                  </span>
                )}
              </button>
            ))}
                ...
                {filteredCombinations.length > 0 
                  ? (
                    <span className="flex items-center gap-2">
                      ...
                      Found {filteredCombinations.length} combination{filteredCombinations.length !== 1 ? 's' : ''}
                    </span>
                  ) : (
                    <span className="flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
          {filteredCombinations.map((combo, index) => (
            <div
              key={combo.id}
              className={`group relative bg-white/90 backdrop-blur-sm rounded-2xl p-6 shadow-xl border border-white/50 hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 hover:rotate-1 ${combo.borderColor} border-l-4 ${
                visibleCards.includes(index) ? 'animate-slide-up opacity-100' : 'opacity-0 translate-y-8'
              }`}
              style={{ 
              ...
              <div className="relative z-10">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    <h2 className="text-xl font-bold text-slate-800 mb-3 flex items-center gap-3 group-hover:text-slate-900 transition-colors">
                      <span className="text-3xl transform group-hover:scale-125 transition-transform duration-300">{combo.icon}</span>
                      <span>{combo.name}</span>
                    </h2>
                    <div className="flex items-center gap-2 mb-3">
                      <span className={`text-sm ${combo.bgColor} ${combo.textColor} px-3 py-2 rounded-full font-bold inline-flex items-center gap-2 shadow-sm`}>
                        {combo.category === 'Avoid' && <AlertTriangle className="w-3 h-3" />}
                        {combo.category === 'Healthy' && <CheckCircle className="w-3 h-3" />}
                        {combo.category === 'Neutral' && <Info className="w-3 h-3" />}
                        {combo.category}
                      </span>
                ...
                {combo.sideEffects && (
                  <div className="mb-4 p-3 bg-red-50/80 rounded-lg border border-red-100">
                    ...
                    <p className="text-sm text-red-700 font-medium">{combo.sideEffects}</p>
                  </div>
                )}
                ...
                {combo.benefits && (
                  <div className="mb-4 p-3 bg-green-50/80 rounded-lg border border-green-100">
                    ...
                    <p className="text-sm text-green-700 font-medium">{combo.benefits}</p>
                  </div>
                )}

                <button className="w-full mt-4 bg-gradient-to-r from-slate-100 to-slate-200 hover:from-green-100 hover:to-green-200 text-slate-700 hover:text-green-800 py-3 px-6 rounded-xl transition-all duration-300 font-bold text-sm shadow-sm hover:shadow-md transform hover:scale-[1.02] active:scale-[0.98]">
                  Learn More
                </button>
              </div>
            </div>
        ...
        <div className="max-w-4xl mx-auto">
          <div className="relative">
            <div className="absolute inset-0 bg-gradient-to-br from-[#89ce4c] via-[#7caf2a] to-[#3dd39c] rounded-2xl blur-2xl opacity-20"></div>
            <div className="relative bg-gradient-to-br from-[#b7f382] via-[#a2db46] to-[#3dd39c] rounded-2xl p-8 text-white shadow-2xl border border-white/20 overflow-hidden">
              ...
              <div className="relative flex items-start gap-4">
                ...
                <div className="flex-shrink-0 hidden md:block">
                  <Sparkles className="w-8 h-8 text-yellow-200 animate-pulse" style={{ animationDelay: '1s' }} />
                </div>
              </div>
            </div>
          </div>
